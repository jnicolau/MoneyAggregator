    ...
    return {
      ...
      link: function link(scope, element, attrs, controller) {
        ...
        var getFalseValue = function() {
          ...
          var value = ($parse(attrs.ngFalseValue)(scope));
          if (angular.isUndefined(falseValue)) {
            value = true;
          }
          return value;
        };
        ...
        var initMaybe = function() {
          ...
          if (!isInit) {
            ...
            var switchClick = getSwitchAttrValue('switchClick');
            element.siblings().click(function() {
              var indeterminate = element.bootstrapSwitch('indeterminate');
              var state = element.bootstrapSwitch('state');
              //console.log("indeterminate: " + indeterminate );
              if (!indeterminate && controller.$modelValue === undefined) {
                controller.$setViewValue(state ? getTrueValue() : getFalseValue());
                switchChange();
              }
            });
